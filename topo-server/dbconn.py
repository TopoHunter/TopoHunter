...
class DBConn:
    ...
    def insert_nodes(self, nodes: list, opr_id: int, host_id: int):
        common.get_logger().debug('Saving Traceroute Results of Nodes (%d Nodes) in Database...' %
                                  len(nodes))
        SQL = """
        INSERT IGNORE INTO Node (node_addr, target_addr, opr_id, host_id)
        VALUES (%s, %s, %s, %s)
        """

        params = []
        for node in nodes:
            params.append((node[0], node[1], opr_id, host_id))
    ...
    def get_nodes(self, opr_id: int, host_id: int):
        SQL = """
        SELECT * FROM Node WHERE opr_id = %s AND host_id = %s
        """
        max_tries = 5
        while True:
            ok = True
    ...
    def insert_edges(self, edges: list, opr_id: int, host_id: int):
        SQL = """
        INSERT INTO Edge (src_addr, dst_addr, target_addr, hop_distance, opr_id, host_id)
        VALUES (%s, %s, %s, %s, %s, %s)
        ...
        params = []
        for edge in edges:
            params.append((edge[0], edge[1], edge[3], edge[2], opr_id, host_id))
        max_tries = 5
        while True:
            ok = True
            max_tries -= 1
    ...
    def get_edges(self, opr_id: int, host_id: int):
        SQL = """
        SELECT * FROM Edge WHERE opr_id = %s AND host_id = %s
        """
        max_tries = 5
        while True:
            ok = True
