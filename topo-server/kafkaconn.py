...
class KafkaConn:
    def __init__(self, vp_name):
        ...
        self.cmd_consumer = KafkaConsumer(common.KAFKA_COMMAND_TOPIC + "-" + self.vp_name,
        ...
        self.res_consumer = KafkaConsumer(common.KAFKA_RESULT_TOPIC + "-" + self.vp_name,
                                          bootstrap_servers=[common.KAFKA_SERVER])
    ...
    def cmd(self, res: dict):
        try:
            self.res_producer.send(common.KAFKA_COMMAND_TOPIC + "-" + self.vp_name, res)
    ...
    def res(self, res: dict):
        try:
            self.res_producer.send(common.KAFKA_RESULT_TOPIC + "-" + self.vp_name, res)
            self.res_producer.flush()
        except Exception as e:
