...
class RedisConn:
    ...
    def set_status(self, opr_id:int, value:str):
        self.set(f"{self.vp_name}:{str(opr_id)}:status", value)

    def get_status(self, opr_id):
        return self.get(f"{self.vp_name}:{str(opr_id)}:status")

    def set_target(self, opr_id:int, value:str):
        self.set(f"{self.vp_name}:{str(opr_id)}:target", value, True)

    def get_target(self, opr_id:int):
        return self.get(f"{self.vp_name}:{str(opr_id)}:target", True)