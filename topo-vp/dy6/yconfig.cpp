...
static struct option long_options[] = {
    ...
    // Add input parameter option 'yarrpid' to modify yarrp_id, default 0x79727036
    {"yarrpid", required_argument, NULL, 'y'},
    // Modify input, IP and split_ttl per line
    {"input2", required_argument, NULL, 0},
    // Modify input, IP and ttl per line
    {"input3", required_argument, NULL, 0},
    // Add input parameter option 'pcap' to save the file path of the grabbed packet
    {"pcap", required_argument, NULL, 0},
    // Add input parameter option 'sqlite' to save the file path of discovered interfaces
    {"sqlite", required_argument, NULL, 0},
    // Maximum permissible number of consecutive anonymous hops
    {"maxanonymous", required_argument, NULL, 0},
    // Whether to detect loopback
    {"noloopbackdetect", no_argument, NULL, 0},
    // Whether to detect reach
    {"noreachdetect", no_argument, NULL, 0},
    // Record probes: IP and TTL
    {"record", required_argument, NULL, 0},
    // Add input parameter option 'exception' to save the file path of exception packets
    {"exception", required_argument, NULL, 0},
    {"validSecs", required_argument, NULL, 0},
    {NULL, 0, NULL, 0},
};
...
YarrpConfig::parse_opts(int argc, char **argv) {
    ...
    while (-1 != (c = getopt_long(argc, argv, "a:b:B:c:CE:F:G:g:i:I:l:m:M:n:o:p:PQr:RsS:t:vVTX:Z:N:y:", long_options, &opt_index))) {
        switch (c) {
        case 'b':
            bgpfile = optarg;
        ...
        case 'X':
            ...
            break;
        case 'y':
            yarrp_id = strtol(optarg, &endptr, 16);
            if (*endptr != '\0' || endptr == optarg) {
                // Conversion failed, default value used
                printf("Invalid yarrp_id format. Using default value.\n");
                yarrp_id = 0x79727036;
                params["Yarrp_ID"] = val_t("0x79727036", true);
            } else {
                params["Yarrp_ID"] = val_t(optarg, true);
            }
            printf("yarrp_id: 0x%08X\n", yarrp_id);
            break;
        case 0:
            if (strcmp("sqlite", long_options[opt_index].name) == 0) {
                sqlite_path = optarg;
                params["Sqlite_Path"] = val_t(sqlite_path, true);
            } else if (strcmp("pcap", long_options[opt_index].name) == 0) {
                pcap = optarg;
                params["Pcap"] = val_t(pcap, true);
            } else if (strcmp("input2", long_options[opt_index].name) == 0) {
                inlist2 = optarg;
                params["Targets"] = val_t(inlist2, true);
            } else if (strcmp("input3", long_options[opt_index].name) == 0) {
                inlist3 = optarg;
                params["Targets"] = val_t(inlist3, true);
            } else if (strcmp("maxanonymous", long_options[opt_index].name) == 0) {
                maxanonymous = strtol(optarg, &endptr, 10);
                params["MaxAnonymous"] = val_t(to_string(maxanonymous), true);
            } else if (strcmp("noloopbackdetect", long_options[opt_index].name) == 0) {
                loopbackdetect = false;
            } else if (strcmp("noreachdetect", long_options[opt_index].name) == 0) {
                reachdetect = false;
            } else if (strcmp("record", long_options[opt_index].name) == 0) {
                record = optarg;
                params["Record"] = val_t(record, true);
                record_out = fopen(record, "a");
            } else if (strcmp("exception", long_options[opt_index].name) == 0) {
                exception = optarg;
                params["Exception"] = val_t(exception, true);
            } else if (strcmp("validSecs", long_options[opt_index].name) == 0) {
                validSecs = strtol(optarg, &endptr, 10);
                params["validSecs"] = val_t(to_string(validSecs), true);
            }
            break;
        default:
            usage(argv[0]);
...
YarrpConfig::usage(char *prog) {
    ...
    << "  --pcap                  Pcap output file (default: none)" << endl
    << "  --noloopbackdetect      Don't detect loopback" << endl
    << "  --noreachdetect         Don't detect reach" << endl
    << "  --record                File path to record probes (IP TTL per line)" << endl
    << "  --exception             Exception pcap output file (default: none)" << endl

    << "Target options:" << endl
    << "  -i, --input             Input target file" << endl
    << "  --input2                Input target file, an IP and its recommended split_ttl per line" << endl
    << "  --input3                Input target file, an IP and its ttl per line" << endl
    ...
    << "  -y, --yarrp_id          Yarrp Id in the Yarrp payload (default: 0x79727036)" << endl
