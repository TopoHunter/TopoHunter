...
class ICMP {
    ...
    virtual struct in6_addr getYarrpTarget() { struct in6_addr temp; return temp; }
...
class ICMP6 : public ICMP {
    public:
    ICMP6(struct ip6_hdr *, struct icmp6_hdr *, uint32_t elapsed, bool _coarse, uint32_t _yarrp_id);
    struct in6_addr *getSrc6() { return &ip_src; }
    struct in6_addr quoteDst6();
    void print();
    void write(FILE **, uint32_t);
    uint32_t yarrp_id;
    struct in6_addr true_target;
    uint16_t make_crafted_cksum(const struct in6_addr, const uint32_t);
