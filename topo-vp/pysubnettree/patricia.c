...
patricia_node_t *patricia_search(patricia_tree_t *patricia,
                                       prefix_t *prefix)
{
  patricia_node_t *node;
  u_char *addr;
...
#ifdef PATRICIA_DEBUG
      if (node->prefix)
        fprintf(stderr, "patricia_search: take right %s/%d\n",
                prefix_toa(node->prefix), node->prefix->bitlen);
      else
        fprintf(stderr, "patricia_search: take right at %u\n", node->bit);
#endif /* PATRICIA_DEBUG */
      node = node->r;
    } else {
#ifdef PATRICIA_DEBUG
      if (node->prefix)
        fprintf(stderr, "patricia_search: take left %s/%d\n",
                prefix_toa(node->prefix), node->prefix->bitlen);
      else
        fprintf(stderr, "patricia_search: take left at %u\n", node->bit);
#endif /* PATRICIA_DEBUG */
      node = node->l;
    }
...
#ifdef PATRICIA_DEBUG
  if (node->prefix)
    fprintf(stderr, "patricia_search: stop at %s/%d\n",
            prefix_toa(node->prefix), node->prefix->bitlen);
  else
    fprintf(stderr, "patricia_search: stop at %u\n", node->bit);
#endif /* PATRICIA_DEBUG */
  return node;
}
